      ...
      <div className="container mx-auto py-10 h-full xl:h-full">
        <div className={`flex flex-col sm:flex-row justify-between ${color ? 'bg-white shadow-inner shadow-slate-300' : 'bg-black shadow-inner shadow-slate-600'}  p-5 gap-2 xl:gap-10`}>
          ...
          <ul className=" flex items-center gap-5 sm:gap-3 md:gap-3 lg:gap-4 overflow-x-auto">
            ...
            <li><button onClick={() => buttonFun('mern stack')} className={`btn btn-xs md:btn-xs lg:btn-sm xl:btn-sm w-16 sm:w-20 md:w-24 lg:w-28 xl:w-36 2xl:w-40 text-white rounded-sm ${color ? 'bg-red-500 hover:bg-red-500' :'bg-yellow-400 hover:bg-yellow-400'} text-[8px] sm:text-[10px] md:text-[11px] lg:text-xs xl:text-sm 2xl:text-base`}>MERN STACK</button></li>
          </ul>
          {/* button end */}
        </div>
          {/* content start */}
